...
void make_title(int);
int week_day(int, int, int);
void echo_line(int, int, int);

int today = 0, dest_mon = 0, current_month = 0;
int main()
{
    int month = 1,          // 当前月
    ...
    printf("输入 年 月 日， 输入全0表示今天\n");
    scanf("%d %d %d", &year, &dest_mon, &today);

    // get time
    time_t time_now = 0;
    struct tm tm_today = {0};
    time(&time_now);
    tm_today = *localtime(&time_now);
    ...
    for(; month <= 12; )
    {
        ...
        // 初始化横排三个月的总天数
        for(i = 0; i < 3; i++)
            total_day[i] = month_day(year, current_month + i);
        // 一直输出，直到三个月都输出完毕
        while(day[0] <= total_day[0] || day[1] <= total_day[1] || day[2] <= total_day[2])
        {
            ...
            for(i = 0; i < 3; i++)
            {
                ...
                week_start[i] = week_day(year, current_month, day[i]);
                // 从开始到结束有几天
    ...
    printf("Today is set to %d year %d month %d day\n", year, dest_mon, today);
}
...
{
    //printf("\033[1;%dm%-4d %d \033[0m", GREEN, (dest_mon - 1) % 3, day[(dest_mon - 1) % 3]);
    int i;
    // 补齐开头的空白
    ...
    {
        if (current_month == dest_mon && start == today)
        {
#ifdef _WIN32
            HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
            ...
            SetConsoleTextAttribute(hConsole, RED|BLUE|GREEN|FOREGROUND_INTENSITY);
#else
            printf("\033[1;47;30m%-4d\033[0m", start);
#endif
